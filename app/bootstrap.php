...
// 		$container->router[] = new CliRouter(array('action' => 'Cli:default'));
		...
		$frontEshopRouter[] = new Route('[<lang=_de [a-z]{0,2}>+<currency=czk [a-z]{3}>/]<eshop>/<category>+c<cid>/[<product>+<presenter><pid>.htm]', array(
			'lang' => array (
					Route::FILTER_TABLE => $langs
			),
			'eshop' => array (
		...
		$frontEshopRouter[] = new Route('[<lang=_de [a-z]{0,2}>+<currency=czk [a-z]{3}>/]<eshop>/<presenter>/<action>[/<id>]', array(
			'lang' => array (
					Route::FILTER_TABLE => $langs
			),
			'eshop' => array (
		...
		$frontRouter[] = new Route('[<lang=_de [a-z]{0,2}>+<currency=czk [a-z]{3}>/][<url>/][<category>+c<cid>/][<tag>+t<tid>/][<article>+a<aid>.htm]', array(
			'lang' => array (
						Route::FILTER_TABLE => $langs
					),
			'presenter' => 'Page',
