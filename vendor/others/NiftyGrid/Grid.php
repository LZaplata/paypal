...
abstract class Grid extends \Nette\Application\UI\Control
{
	...
	protected function attached($presenter)
	{
		...
		if (!$presenter->user->loggedIn && $this->presenter->moduleName == 'Admin') {
			$presenter->redirect(':Admin:Sign:in');
		}
	...
	protected function createComponentGridForm()
	{
		...
		$form[$this->name]['rowForm']['send']->getControlPrototype()->addClass("grid-editable btn btn-primary btn-sm");

		$form[$this->name]->addContainer("filter");
		$form[$this->name]['filter']->addSubmit("send","Filtrovat")
			->setValidationScope(FALSE)
			->getControlPrototype()->class('btn btn-default btn-sm');
		...
		if ($this->presenter->presenterName == 'Products') {
			$form[$this->name]['action']->addSelect('categories_id', null, $this->presenter->categories)
				->getControlPrototype()
				->class('form-control input-sm');
		}
		if ($this->presenter->presenterName == 'Categories') {
			$form[$this->name]['action']->addSelect('pid', null, $this->presenter->getCategoriesSelect())
				->getControlPrototype()
				->class('form-control input-sm');
		}
		...
		$form[$this->name]['perPage']->addSelect("perPage","Záznamů na stranu:", $this->perPageValues)
			->getControlPrototype()
			->addClass("grid-changeperpage form-control input-sm")
				...
				if(isset($this['gridForm'][$this->name]['rowForm'][$name])){
					...
					if($input instanceof \Nette\Forms\Controls\SelectBox){
						...
						if(in_array($row[$name], $items)){
							$select[$name] = array_search($row[$name], $items);
						}
					}
				}
			}
			$this['gridForm'][$this->name]['rowForm']->setDefaults($row);
			$this['gridForm'][$this->name]['rowForm']->setDefaults($select);
