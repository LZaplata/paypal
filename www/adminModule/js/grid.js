$(function(){	
    ...
    $(/*".grid.categories .grid-select-all, .grid.categories .grid-action-checkbox, .grid.accounts .grid-select-all, .grid.accounts .grid-action-checkbox"*/".grid .grid-select-all, .grid .grid-action-checkbox").live("click", function () {
    	var checkboxes =  $(this).parents().eq(3).find(".grid-action-checkbox:checked");
    	/*console.log(checkboxes.length);
    	if (checkboxes.length > 0) {
    		$(this).parents().eq(4).find("tfoot").show();
    	}
    	else {
    		$(this).parents().eq(4).find("tfoot").hide();
    	}*/
    });
    ...
    $(".grid-gridForm").live("submit", function(event){
        ...
        if($(button).data("select")){
            ...
            if ($(button).attr("name") != 'grid[filter][send]') {
                event.preventDefault();
            	$.post(this.action, $(this).serialize()+"&"+$(button).attr("name")+"="+$(button).val());
            }
        }
...
//    setDatepicker();
    ...
    $("#frm-grid-action-action_name").live("change", function (e) {
    	if ($(this).val() == "add") {
    		$("#frm-grid-gridForm-grid-action-categories_id").show();
    	}
    	else {
    		$("#frm-grid-gridForm-grid-action-categories_id").hide();
    	}
    	
    	if ($(this).val() == "move") {
    		$("#frm-grid-gridForm-grid-action-pid").show();
    	}
    	else {
    		$("#frm-grid-gridForm-grid-action-pid").hide();
    	}
    });
});